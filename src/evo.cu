...
int main(int argc, char** argv){
    if(argc<5){
       //printf("usage: %s <g_beta> <total_steps> <trotter_stepsize> <outfile> [--init <initfile>] [--test-eigvectors]\n",argv[0]);  //TODO: test eigenvector
...
//        int ectrl=fscanf(fl_init,"%lg\n",&eigenval); (void)ectrl; //TODO: input eigenvalue if testing eigenvector
...
//        DEBUG_READ_STATE();
        ...
        if(with_initialization){
            ...
            for(size_t jj=0; jj<dim; ++jj){
                double initial_coeff=re_coeff[jj]*re_coeff[jj]+im_coeff[jj]*im_coeff[jj];
                double evo_coeff=suqa::state.data_re[jj]*suqa::state.data_re[jj]+suqa::state.data_im[jj]*suqa::state.data_im[jj];
